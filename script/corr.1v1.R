...
zdf = data.frame(zdf)
rownames_zdf = zdf[,1]
if(rownames_zdf[1]==rownames_zdf[2]){
	rownames_zdf[2] = paste(rownames_zdf[2],"(2)",sep="")
}
colnames(zdf)= names
zdf = zdf[,-1]
zdf=as.matrix(zdf)
...
if(!is.na(dataset)){
  ...
  sample_bank=data.frame(sample_bank)
  print(head(sample_bank))
...
colnames(rpkm_data_2) = rownames_zdf[1:2]
#print(rownames(rpkm_data_2)[1:5])
